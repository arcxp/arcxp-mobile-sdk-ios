...
module$exports$omid$complianceVerificationScript$ComplianceVerificationClient.prototype.serialize_ = function(a, b) {
  ...
  for (d in a) {
    if (a.hasOwnProperty(d)) {
      var e = b ? b + "[" + d + "]" : d, f = a[d];
      "videoElement" === d && (f = "DOM Video Element - Present but not parsed to avoid parse error");
      "slotElement" === d && (f = "DOM Slot Element - Present but not parsed to avoid parse error");
      ...
      "accessMode" === d && c.push(encodeURIComponent(b ? b + "[friendlyToTop]" : "friendlyToTop") + "=" + JSON.stringify((0,module$exports$omid$common$windowUtils.isTopWindowAccessible)(window)));
    }
  }
  return c.join("&");
};
